...
# Let's fix the request body and make sure we have correct quoting
echo "1. Register"
curl -s -X POST http://127.0.0.1:8000/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"email": "test3@example.com", "password": "password123", "full_name": "Test User 3"}' | python3 -m json.tool

echo -e "\n\n2. Login"
curl -v -c cookies.txt -X POST http://127.0.0.1:8000/api/auth/login \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data-urlencode "username=test3@example.com" \
